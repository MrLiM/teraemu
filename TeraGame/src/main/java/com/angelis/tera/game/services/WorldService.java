...
public class WorldService extends AbstractService {
    ...
    public Set<TeraGameConnection> getAllConnections() {
    ...
    public List<Player> computePlayerObserversForPlayer(Player player) {
        List<Player> playersObservers = this.getPlayersByMap(player.getWorldPosition().getMapId());
    ...
    public List<Player> computePlayerObservablesForPlayer(Player player) {
        List<Player> playersInMap = this.getPlayersByMap(player.getWorldPosition().getMapId());
    ...
    public List<Player> getPlayerObserversForPlayer(Player player) {
        List<Player> playerObservers = this.getPlayersByMap(player.getWorldPosition().getMapId());
    ...
    public void updatePlayerObservers(Player player) {
        ...
        while (iterator.hasNext()) {
            ...
            if (!this.onlinePlayers.contains(player) || (playerObservers.indexOf(observer) == -1 && teraObjectObservers.indexOf(observer) == -1)) {
                iterator.remove();
    ...
    public void updatePlayerObservables(Player player) {
        ...
        for (Player observer : playerObservers) {
            if (!this.onlinePlayers.contains(player) || playerObservables.indexOf(observer) == -1) {
