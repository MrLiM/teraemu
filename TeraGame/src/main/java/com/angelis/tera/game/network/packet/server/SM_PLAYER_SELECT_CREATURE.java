...
public class SM_PLAYER_SELECT_CREATURE extends TeraServerPacket {
    ...
    public SM_PLAYER_SELECT_CREATURE(Creature creature) {
    ...
    protected void writeImpl(TeraGameConnection connection, ByteBuffer byteBuffer) {
        ...
        int effectShift = byteBuffer.position();
        
        writeH(byteBuffer, 0); //first Abnormal shift
        writeD(byteBuffer, 0); //unk
        writeUid(byteBuffer, this.creature);
        writeF(byteBuffer, 1);// TODO (Creature.LifeStats.Hp/(Creature.MaxHp/100F))/100F (percent / 100)
        writeD(byteBuffer, 0);
